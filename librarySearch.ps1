...
function CheckLibrary
{
    ...
    param(
     ...
     for($i = 1; $i -lt $trs.length; $i++){
        ...
        $id = ($trs[$i].lastChild.firstChild.id | Select-String "availabilityDiv(.*)").Matches.Groups[1].Value
        $LibraryIds[$lname] = $id
     }
 

     $availabilityJson = ($results.Scripts[-1].InnerHtml | Select-String "(?smi)^parseDetailAvailabilityJSON\(({.*?})\);").Matches[0].Groups[1]
